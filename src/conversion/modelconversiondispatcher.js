...
export default class ModelConversionDispatcher {
	...
	/**
	 * Fires events for model converters basing on diff obtained from {@link module:engine/model/differ~Differ Differ}.
	 *
	 * @param {Array.<Object>} diff Diff between old model state and current model state.
	 */
	convertBufferedChanges( diff ) {
	...
	/**
	 * Fires {@link #event:addMarker addMarker events}  basing on a set of changes
	 * obtained from {@link module:engine/model/differ~Differ Differ}.
	 */
	addMarkers( markersChanges ) {
		for ( const change of markersChanges ) {
			this.convertMarker( 'addMarker', change.name, change.range );
		}
	}

	/**
	 * Fires {@link #event:removeMarker removeMarker events} basing on a set of changes
	 * obtained from {@link module:engine/model/differ~Differ Differ}.
	 *
	 * @param {Array.<Object>} markersChanges
	 */
	removeMarkers( markersChanges ) {
		for ( const change of markersChanges ) {
			this.convertMarker( 'removeMarker', change.name, change.range );
		}
	}
	...
	convertAttribute( range, key, oldValue, newValue ) {
		...
		const consumable = this._createAttributeConsumable( range, `attribute:${ key }` );
	...
	/**
	 * Starts marker conversion. Fires {@link #event:addMarker addMarker} or {@link #event:removeMarker removeMarker} event
	 * with provided data.
	 *
	 * @fires addMarker
	 * @fires removeMarker
	...
	convertMarker( type, name, range ) {
		...
		this.fire( type + ':' + name, { markerName: name, markerRange: range }, this.conversionApi );
	}
	 ...
	 * assuming that an attribute has changed on that range.
	 *
	 * @private
	 ...
	 * @returns {module:engine/conversion/modelconsumable~ModelConsumable} Values to consume.
	 */
	_createAttributeConsumable( range, type ) {
		const consumable = new Consumable();
