		...
		describe( 'add', () => {
			...
			it( 'should append child controller\'s view to parent controller\'s view', () => {
				...
				return parentController.init()
					...
					.then( () => {
						sinon.assert.calledOnce( spy );
						sinon.assert.calledWithExactly( spy, sinon.match.instanceOf( EventInfo ), childController.view, 0 );
					} );
			} );
			...
			it( 'should append child controller\'s view to parent controller\'s view at given index', () => {
				...
				return parentController.init()
					...
					.then( () => {
						expect( parentController.view.regions.get( 'x' ).views.get( 0 ) ).to.be.equal( childView2 );
						expect( parentController.view.regions.get( 'x' ).views.get( 1 ) ).to.be.equal( childView1 );
					} );
			} );
		} );
			...
			it( 'should remove child controller\'s view from parent controller\'s view', () => {
				...
				return parentController.init()
					.then( () => {
						collection.remove( childController );
						sinon.assert.calledOnce( spy );
						sinon.assert.calledWithExactly( spy, sinon.match.instanceOf( EventInfo ), childController.view );
					} );
			} );
		} );
	} );
