...
describe( 'advanced-converters', () => {
	...
	beforeEach( () => {
		...
		const editing = new EditingController( modelDoc );

		mapper = editing.mapper;
	...
	describe( 'custom attribute handling for given element', () => {
		...
		it( 'should convert model text with linkHref and linkTitle to view', () => {
			...
			modelDoc.enqueueChanges( () => {
				modelDoc.batch().setAttribute( 'linkHref', 'bar.html', range );
				modelDoc.batch().setAttribute( 'linkTitle', 'Bar title', range );
			} );
			...
			modelDoc.enqueueChanges( () => {
				modelDoc.batch().remove( ModelRange.createFromParentsAndOffsets( modelRoot, 0, modelRoot, 1 ) );
			} );
			...
			modelDoc.enqueueChanges( () => {
				...
				modelDoc.batch().removeAttribute( 'linkTitle', range );
			} );
			...
			modelDoc.enqueueChanges( () => {
				range = ModelRange.createIn( modelRoot );
				modelDoc.batch().removeAttribute( 'linkHref', range );
			} );
		...
		it( 'should convert quote model element with linkHref and linkTitle attribute to view', () => {
			...
			modelDoc.enqueueChanges( () => {
				modelDoc.batch().removeAttribute( 'linkTitle', modelElement );
				modelDoc.batch().setAttribute( 'linkHref', 'bar.html', modelElement );
			} );
			...
			modelDoc.enqueueChanges( () => {
				modelDoc.batch().removeAttribute( 'linkHref', modelElement );
			} );
	...
	describe( 'universal converter', () => {
		beforeEach( () => {
			...
			modelDispatcher.on( 'attribute', setAttribute(), { priority: 'lowest' } );
